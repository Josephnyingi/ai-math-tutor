...
**Status: trained and committed.** Adapter weights are in `tutor/adapters/distilgpt2-numeracy-lora/` (1.6 MB safetensors). Full TinyLlama production training runs on Colab T4 in ~25 min.
...
**Training (production, TinyLlama, Colab T4, ~25 min):**

```bash
pip install peft transformers bitsandbytes accelerate datasets trl
python3 scripts/train_lora.py --epochs 3 --batch 4
# Then merge + quantise:
